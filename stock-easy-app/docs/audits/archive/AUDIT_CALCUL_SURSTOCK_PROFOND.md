...
1. **Cas limite** : Si `salesPerDay = 0`, on ne peut pas calculer l'excédent en unités.
   
   **✅ Solution implémentée** : Approche hybride dans `calculateOverstockExcessValue()` :
   - Si `salesPerDay > 0` : Calcul de l'excédent au-delà du seuil (approche 2 - précise)
   - Si `salesPerDay = 0` : Valeur totale du stock (approche 1 - fallback)
   
   **Justification** : Un produit sans ventes avec du stock est considéré comme entièrement en surstock car il n'y a aucune perspective d'écoulement naturel du stock.
