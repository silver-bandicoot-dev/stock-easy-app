...
export const onSuccess = async ({ params, record, logger, api, connections }) => {
  try {
    ...
    // Sync the new location to Supabase warehouse
    ...
    logger.info({ 
      ...
      name: record.name
    }, 'Created warehouse for new Shopify location');
    ...
    await api.syncLog.create({
      ...
      status: 'success',
      shopifyId: record.id,
      message: `Created warehouse for location ${record.name}`,
      payload: {
        shopifyLocationId: record.id,
        warehouseId: warehouseId,
        name: record.name,
    ...
    }, 'Failed to create warehouse for Shopify location');
    ...
    await api.syncLog.create({
      ...
      status: 'error',
      shopifyId: record.id,
      message: `Failed to create warehouse for location ${record.name}: ${error.message}`,
      payload: {
        shopifyLocationId: record.id,
        error: error.message
      }
